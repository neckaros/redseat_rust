...
use crate::{
    domain::{
        media::{MediaForUpdate, DEFAULT_MIME},
        plugin::{PluginForAdd, PluginForInstall, PluginForUpdate, PluginRepoAdd},
    },
    error::RsError,
    model::{credentials::CredentialForAdd, plugins::PluginQuery, users::ConnectedUser},
    tools::{
        array_tools::value_to_hashmap,
        convert::{convert_from_to, ConvertFileSource},
        http_tools::download_latest_wasm,
    },
...
async fn handler_install(
    State(mc): State<ModelController>,
    user: ConnectedUser,
    Json(plugin): Json<PluginForInstall>,
) -> Result<Json<Value>> {
...
async fn handler_upload_plugin(
    State(mc): State<ModelController>,
    user: ConnectedUser,
...
async fn handler_upload_repo_plugin(
    State(mc): State<ModelController>,
    user: ConnectedUser,
    Json(plugin): Json<PluginRepoAdd>,
) -> Result<Json<Value>> {
...
async fn handler_reload_plugins(
    State(mc): State<ModelController>,
    user: ConnectedUser,
) -> Result<Json<Value>> {
...
async fn handler_urlrequest(
    ...
    let wasm = mc
        .exec_request(request, None, false, None, &user, None)
        .await?;
    let body = match wasm {
...
async fn handler_exchange_token(
    Path(plugin_id): Path<String>,
    State(mc): State<ModelController>,
    user: ConnectedUser,
...
async fn handler_patch(
    ...
    State(mc): State<ModelController>,
    user: ConnectedUser,
    Json(update): Json<PluginForUpdate>,
) -> Result<(Json<Value>)> {
...
async fn handler_convert(
    State(mc): State<ModelController>,
    user: ConnectedUser,
    Query(query): Query<ConvertParams>,
    mut multipart: Multipart,
) -> Result<Vec<u8>> {
    ...
    while let Some(field) = multipart.next_field().await.unwrap() {
        ...
        if name == "info" {
            ...
            let filename = field.file_name().unwrap().to_string();
            let mime = if query.from.is_none() {
