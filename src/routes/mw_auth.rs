...
pub async fn mw_must_be_admin(user: ConnectedUser, req: Request, next: Next) -> Result<Response> {
    match user {
        ...
        ConnectedUser::Anonymous | ConnectedUser::Guest(_) | ConnectedUser::UploadKey(_) => {
            return Err(Error::Forbiden)
        }
...
pub async fn mw_token_resolver(
    ...
    if let Some(token) = &auth.token {
        if let Some(server_id) = server_id {
            ...
            match user {
                ...
                Err(crate::model::error::Error::UserNotFound(_id)) => {
                    Ok(ConnectedUser::Guest(GuestUser {
                        id: claims.sub.clone(),
