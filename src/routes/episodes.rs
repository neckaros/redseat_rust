...
use crate::{
    domain::{
        ...
        media::{FileEpisode, Media, MediaForUpdate},
        progress,
...
use rs_plugin_common_interfaces::{
    ...
    ElementType, ImageType, MediaType,
};
...
pub fn routes(mc: ModelController) -> Router {
    Router::new()
        ...
        .route(
            "/seasons/:season/searchstream",
            get(handler_lookup_season_stream),
        )
        .route(
            "/seasons/:season/episodes",
            get(handler_list_season_episodes),
        )
        .route("/seasons/:season/episodes/:number", get(handler_get))
        .route(
            "/seasons/:season/episodes/:number",
            post(handler_post_episode),
        )
        .route("/seasons/:season/episodes/:number", patch(handler_patch))
        .route("/seasons/:season/episodes/:number", delete(handler_delete))
        .route(
            "/seasons/:season/episodes/:number/image",
            get(handler_image),
        )
        .route(
            "/seasons/:season/episodes/:number/search",
            get(handler_lookup),
        )
        .route(
            "/seasons/:season/episodes/:number/searchstream",
            get(handler_lookup_stream),
        )
        .route(
            "/seasons/:season/episodes/:number/search",
            post(handler_lookup_add),
        )
        .route(
            "/seasons/:season/episodes/:number/medias",
            get(handler_medias),
        )
        .route(
            "/seasons/:season/episodes/:number/progress",
            get(handler_progress_get),
        )
        .route(
            "/seasons/:season/episodes/:number/progress",
            post(handler_progress_set),
        )
        .route(
            "/seasons/:season/episodes/:number/rating",
            get(handler_rating_get),
        )
        .route(
            "/seasons/:season/episodes/:number/rating",
            patch(handler_rating_set),
        )
        .route(
            "/seasons/:season/episodes/:number/watched",
            get(handler_watched_get),
        )
        .route(
            "/seasons/:season/episodes/:number/watched",
            post(handler_watched_set),
        )
        .route(
            "/seasons/:season/episodes/:number/watched",
            delete(handler_watched_delete),
        )
...
async fn handler_list(
    ...
    user: ConnectedUser,
    Query(mut query): Query<EpisodeQuery>,
) -> Result<Json<Value>> {
...
async fn handler_refresh(
    ...
    State(mc): State<ModelController>,
    user: ConnectedUser,
    Query(_query): Query<EpisodeQuery>,
) -> Result<Json<Value>> {
...
async fn handler_lookup_season(
...
async fn handler_list_season_episodes(
    Path((library_id, serie_id, season)): Path<(String, String, u32)>,
    State(mc): State<ModelController>,
    user: ConnectedUser,
    Query(mut query): Query<EpisodeQuery>,
...
async fn handler_post_episode(
    Path((library_id, serie_id, season, number)): Path<(String, String, u32, u32)>,
    State(mc): State<ModelController>,
    user: ConnectedUser,
    Json(mut insert): Json<Episode>,
) -> Result<Json<Value>> {
...
async fn handler_patch(
    ...
    State(mc): State<ModelController>,
    user: ConnectedUser,
    Json(update): Json<EpisodeForUpdate>,
) -> Result<Json<Value>> {
    let new_credential = mc
        .update_episode(&library_id, serie_id, season, number, update, &user)
        .await?;
    Ok(Json(json!(new_credential)))
}
...
async fn handler_delete(
    ...
    let library = mc
        .remove_episode(&library_id, &serie_id, season, number, &user)
        .await?;
    let body = Json(json!(library));
    Ok(body)
}
...
async fn handler_lookup_season_stream(
    Path((library_id, serie_id, season)): Path<(String, String, u32)>,
    State(mc): State<ModelController>,
    user: ConnectedUser,
) -> Result<Sse<impl Stream<Item = std::result::Result<Event, Infallible>>>> {
...
async fn handler_medias(
    ...
    let libraries = mc
        .get_medias(
            &library_id,
            MediaQuery {
                series: vec![format!("{}|{:04}|{:04}", serie_id, season, number)],
                ..Default::default()
            },
            &user,
        )
...
async fn handler_rating_get(
    ...
    let rating = mc
        .get_media_rating(&library_id, ElementType::Episode, episode_ref, &user)
        .await?;
    Ok(Json(json!(rating)))
}
...
async fn handler_watched_get(
    ...
    let progress = mc
        ...
        .into_iter()
        .next()
        .ok_or(Error::NotFound(format!("Unable to get best watched")))?;
    Ok(Json(json!(progress)))
}
...
async fn handler_image(
    ...
    if let Ok(reader_response) = reader_response {
        ...
        Err(Error::NotFound(format!(
            "Unable to find episode image with defaulting: {} {} {:?}",
...
async fn handler_post_image(
    ...
    while let Some(field) = multipart.next_field().await.unwrap() {
        ...
        mc.update_serie_image(&library_id, &tag_id, &query.kind, reader, &user)
            .await?;
    }
