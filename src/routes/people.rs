...
async fn handler_patch(
    ...
    State(mc): State<ModelController>,
    user: ConnectedUser,
    Json(update): Json<PersonForUpdate>,
) -> Result<Json<Value>> {
...
async fn handler_image(
    ...
    let reader_response = mc
        .person_image(&library_id, &tag_id, query.kind, query.size, &user)
        .await?;
...
async fn handler_post_image(
    ...
    while let Some(field) = multipart.next_field().await.unwrap() {
        ...
        mc.update_person_image(&library_id, &person_id, &None, reader, &user)
            .await?;
    }
...
async fn handler_get_unassigned_faces(
    ...
    let faces = mc
        .get_all_unassigned_faces(&library_id, limit, query.created_before, &user)
        .await?;
    Ok(Json(json!(faces)))
}
...
async fn handler_assign_face_to_person(
    ...
    let unassigned_count = mc
        .unassign_faces_from_person(&library_id, &payload.face_ids, &user)
        .await?;

    // Then assign all faces to the new person
    let assigned_count = mc
        .assign_unassigned_faces_to_person(
            &library_id,
            &payload.face_ids,
            &payload.person_id,
            &user,
...
async fn handler_start_face_recognition_task(
    ...
    use crate::tools::scheduler::{
        face_recognition::FaceRecognitionTask, RsSchedulerWhen, RsTaskType,
    };
    ...
    mc.scheduler
        .add(RsTaskType::Face, RsSchedulerWhen::At(0), task)
        .await?;
