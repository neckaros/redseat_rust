use crate::{
    domain::{
        view_progress::ViewProgressForAdd,
        watched::{Watched, WatchedForAdd, WatchedForDelete},
    },
    model::{
        users::{ConnectedUser, HistoryQuery, InvitationRedeemer, UserRole, ViewProgressQuery},
        ModelController,
    },
    Result,
};
...
async fn handler_list_history_all(
    State(mc): State<ModelController>,
    user: ConnectedUser,
) -> Result<Json<Value>> {
...
async fn handler_add_all_history(
    State(mc): State<ModelController>,
    user: ConnectedUser,
    Json(watcheds): Json<Vec<WatchedForAdd>>,
) -> Result<Json<Value>> {
...
async fn handler_add_history(
    State(mc): State<ModelController>,
    user: ConnectedUser,
    Json(watched): Json<WatchedForAdd>,
) -> Result<Json<Value>> {
    mc.add_watched(watched, &user, None).await?;
    let body = Json(json!({"ok": true}));
...
async fn handler_delete_history(
    State(mc): State<ModelController>,
    user: ConnectedUser,
    Json(watched): Json<WatchedForDelete>,
) -> Result<Json<Value>> {
...
async fn handler_add_progress(
    State(mc): State<ModelController>,
    user: ConnectedUser,
    Json(watched): Json<ViewProgressForAdd>,
) -> Result<Json<Value>> {
...
async fn handler_hide_library(
    State(mc): State<ModelController>,
    user: ConnectedUser,
    Json(body): Json<HideLibraryRequest>,
) -> Result<Json<Value>> {
    mc.hide_library(body.library, &user).await?;
    Ok(Json(json!({"ok": true})))
}

async fn handler_unhide_library(
    State(mc): State<ModelController>,
    user: ConnectedUser,
    Json(body): Json<HideLibraryRequest>,
) -> Result<Json<Value>> {
...
async fn handler_invitation(
    State(mc): State<ModelController>,
    user: ConnectedUser,
    Query(query): Query<InvitationRedeemer>,
) -> Result<Json<Value>> {
    let library = mc.redeem_invitation(query.code, user.clone()).await?;

    let body = Json(json!(
        json!({"library": library, "uid": user.user_id()?, "name": user.user_name()?, "user": user})
    ));
    Ok(body)
}
