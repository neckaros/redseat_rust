...
use crate::{domain::backup::BackupWithStatus, error::RsError, model::{backups::{BackupForAdd, BackupForUpdate}, users::ConnectedUser, ModelController}, plugins::sources::error::SourcesError, tools::scheduler::{backup::BackupTask, RsSchedulerTask}, Result};
...
async fn handler_get(Path(backup_id): Path<String>, State(mc): State<ModelController>, user: ConnectedUser) -> Result<Json<Value>> {
	let backup = mc.get_backup_with_status(&backup_id, &user).await?.ok_or(SourcesError::UnableToFindBackup(backup_id, "handler_get".to_string()))?;
...
async fn handler_get_last_backup_media(Path((backup_id, media_id)): Path<(String, String)>, State(mc): State<ModelController>, user: ConnectedUser) -> Result<Response<Body>> {
	...
	let last = backups.last().ok_or(SourcesError::UnableToFindBackup(backup_id, "handler_get_last_backup_media".to_string()))?;
