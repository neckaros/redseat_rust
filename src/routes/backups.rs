...
async fn handler_get(
    ...
    let backup = mc.get_backup_with_status(&backup_id, &user).await?.ok_or(
        SourcesError::UnableToFindBackup(backup_id, "handler_get".to_string()),
    )?;
...
async fn handler_patch(
    ...
    State(mc): State<ModelController>,
    user: ConnectedUser,
    Json(update): Json<BackupForUpdate>,
) -> Result<Json<Value>> {
