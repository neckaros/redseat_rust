...
use crate::{
    ...
    server::{
        check_unregistered, get_config, get_install_url, get_server_file_path, get_server_id,
        get_web_url, update_config, PublicServerInfos,
    },
    tools::{
...
async fn handler_infos(State(mc): State<ModelController>) -> Result<Json<Value>> {
    let admin_users = mc
        .get_users(&ConnectedUser::ServerAdmin)
        .await?
        .into_iter()
        .filter(|u| u.is_admin())
        .collect::<Vec<_>>();
...
async fn handler_own(
    State(mc): State<ModelController>,
    Query(query): Query<OwnQuery>,
) -> Result<Redirect> {
    let admin_users = mc
        .get_users(&ConnectedUser::ServerAdmin)
        .await?
...
async fn handler_install() -> Result<Redirect> {
    log_info(
        ...
        "Getting install request checking that server is unregistered".to_string(),
    );
