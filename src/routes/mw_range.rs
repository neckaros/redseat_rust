...
pub async fn mw_range(
    headers: HeaderMap,
    query: Query<RangeParam>,
    mut req: Request,
    next: Next,
) -> Result<Response> {
...
pub fn parse_range(range: String) -> Result<RangeDefinition> {
    ...
    let range: Vec<Option<u64>> = range
        .replace("bytes=", "")
        .split("-")
        .map(|e| e.parse::<u64>().ok())
        .collect();
    Ok(RangeDefinition {
        start: range.get(0).unwrap_or(&None).clone(),
        end: range.get(1).unwrap_or(&None).clone(),
    })
}
