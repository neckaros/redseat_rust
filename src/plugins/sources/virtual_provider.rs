...
use crate::{
    ...
    plugins::{sources::local_provider_for_library, PluginManager},
...
use super::{
    ...
    AsyncReadPinBox, AsyncSeekableWrite, BoxedStringFuture, FileStreamResult, Source, SourceRead,
};
...
impl Source for VirtualProvider {
    ...
    async fn new_from_backup(_: Backup, _: ModelController) -> RsResult<Self> {
        Err(crate::Error::NotImplemented(
            "Virtual providers can't be used for backup".to_string(),
        ))
    }
    async fn init(&self) -> SourcesResult<()> {
        let source = local_provider_for_library(&self.library)
            .await
            .map_err(|_| {
                SourcesError::Other(
                    "Unable to get local provider source during init of virtual library"
                        .to_string(),
                )
            })?;
        source.init().await?;
    ...
    async fn writer(
        ...
        Err(crate::Error::NotImplemented(
            "Writer not implemented for virtual provider".to_string(),
        ))
    }
