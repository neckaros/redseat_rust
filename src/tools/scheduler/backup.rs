        ...
        for backup in backups {
            ...
            if existing {
                log_info(
                    ...
                    format!("Backup {} already procession", backup.id),
                );
                continue;
            }
            ...
            if let Some(library_id) = &backup.library {
                ...
                let server_db_path =
                    get_server_file_path_array(vec![&"dbs", &format!("db-{}.db", library_id)])
                        .await
                        .map_err(|_| {
                            RsError::Error("Unable to get database path for backup".to_string())
                        })?;
                let db_backup = mc
                    .upload_backup_path(
                        backup.clone(),
