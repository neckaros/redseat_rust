        ...
        for library in libraries {
            ...
            for serie in series {
                ...
                if let Err(refreshed) = refreshed {
                    log_error(
                        crate::tools::log::LogServiceType::Scheduler,
                        format!("Error refreshing serie {}: {:#}", serie.name, refreshed),
                    );
                } else {
                    log_info(
                        crate::tools::log::LogServiceType::Scheduler,
                        format!("Refreshed serie {}", serie.name),
                    );
                }
                if serie.status != Some(SerieStatus::Ended)
                    && serie.status != Some(SerieStatus::Canceled)
                {
                    let episodes = mc
                        .refresh_episodes(&library.id, &serie.id, &connected_user)
                        .await;
                    if let Err(error) = episodes {
                        log_error(
                            crate::tools::log::LogServiceType::Scheduler,
                            format!(
                                "Error refreshing serie episodes {}: {:#}",
                                serie.name, error
                            ),
                        );
            ...
            for movie in movies {
                let refreshed = mc
                    .refresh_movie(&library.id, &movie.id, &connected_user)
                    .await;
                if let Err(refreshed) = refreshed {
                    log_error(
                        crate::tools::log::LogServiceType::Scheduler,
                        format!("Error refreshing movie {}: {:#}", movie.name, refreshed),
                    );
                } else {
                    log_info(
                        crate::tools::log::LogServiceType::Scheduler,
                        format!("Refreshed movie {}", movie.name),
                    );
                }
            }
            ...
            let people: Vec<Person> = mc
                .get_people(&library.id, PeopleQuery::new_empty(), &connected_user)
                .await?
                .into_iter()
                .filter(|s| s.trakt.is_some())
                .collect();
            let people: Vec<Person> = if let Some(last_update) = last_update {
                ...
                if let Ok(to_refresh) = to_refresh {
                    ...
                    log_info(
                        crate::tools::log::LogServiceType::Scheduler,
                        "Too many page will refresh all people".to_owned(),
                    );
                    people
                }
            } else {
            ...
            for person in people {
                let refreshed = mc
                    .refresh_person(&library.id, &person.id, &connected_user)
                    .await;
                if let Err(refreshed) = refreshed {
                    log_error(
                        crate::tools::log::LogServiceType::Scheduler,
                        format!("Error refreshing movie {}: {:#}", person.name, refreshed),
                    );
                } else {
                    log_info(
                        crate::tools::log::LogServiceType::Scheduler,
                        format!("Refreshed person {}", person.name),
                    );
                }
            }
