...
use crate::model::people::PeopleQuery;
        ...
        for library in libraries {
            ...
            let series: Vec<Serie> = if let Some(last_update) = last_update {
                ...
                if let Ok(to_refresh) = to_refresh {
                    ...
                    log_info(crate::tools::log::LogServiceType::Scheduler, "Too many page will refresh all shows".to_owned());
                    series
                }
            } else {
                series
            ...
            let movies: Vec<Movie> = if let Some(last_update) = last_update {
                ...
                if let Ok(to_refresh) = to_refresh {
                    ...
                    log_info(crate::tools::log::LogServiceType::Scheduler, "Too many page will refresh all movies".to_owned());
                    movies
                }
            } else {
                movies
            ...
            // People
            let refresh_path = "settings/trakt_people_refresh.txt";
            let source = mc.library_source_for_library(&library.id).await?;
            let last_update = if let Ok(mut data) = source.get_file_library(refresh_path,).await {
                let mut buffer = String::new();
                data.read_to_string(&mut buffer).await?;
            ...
            let people: Vec<Person> = mc.get_people(&library.id, PeopleQuery::new_empty(), &connected_user).await?.into_iter().filter(|s| s.trakt.is_some()).collect();
            let people: Vec<Person> = if let Some(last_update) = last_update {
                let to_refresh = mc.trakt.movies_refreshed(last_update).await;
                if let Ok(to_refresh) = to_refresh {
                    people.into_iter().filter(|s| to_refresh.contains(&s.trakt.unwrap_or(0))).collect()
                } else {
                    log_info(crate::tools::log::LogServiceType::Scheduler, "Too many page will refresh all people".to_owned());
                    people
                }
            } else {
                people
            ...
            for person in people {
                let refreshed = mc.refresh_person(&library.id, &person.id, &connected_user).await;
                if let Err(refreshed) = refreshed {
                    log_error(crate::tools::log::LogServiceType::Scheduler, format!("Error refreshing movie {}: {:#}", person.name, refreshed));
                } else {
                    log_info(crate::tools::log::LogServiceType::Scheduler, format!("Refreshed person {}", person.name));
                }
            }
