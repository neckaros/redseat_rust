...
self.add_out_option("+faststart");
    ...
    pub fn set_aspect_ratio(&mut self, aspect_ratio: String, alignment: VideoAlignment) -> RsResult<&mut Self> {
        ...
        if num == denum {
            ...
            let crop_expr = match alignment {
                VideoAlignment::Center => format!(
                    "[#input#]crop='if(gte(iw,ih),ih*({num}/{denum}),iw):if(gte(iw, ih),ih,iw*({denum}/{num}))'"
                ),
                VideoAlignment::Left => format!(
                    "[#input#]crop='if(gte(iw,ih),ih*({num}/{denum}),iw):if(gte(iw,ih),ih,iw*({denum}/{num})):0:0'"
                ),
                VideoAlignment::Right => format!(
                    "[#input#]crop='if(gte(iw,ih),ih*({num}/{denum}),iw):if(gte(iw,ih),ih,iw*({denum}/{num})):if(gte(iw,ih),iw - ih*({num}/{denum}),0):0'"
                ),
            };
            println!("align {:?}, crop: {}", alignment, crop_expr);
            self.add_video_effect(crop_expr);
        }
        Ok(self)
    }
