...
pub fn replace_add_remove_from_array<T: PartialEq + Clone>(
...
pub fn add_remove_from_array<T: PartialEq + Clone>(
    existing: Option<Vec<T>>,
    add: Option<Vec<T>>,
    remove: Option<Vec<T>>,
) -> Option<Vec<T>> {
...
mod tests {
    ...
    async fn add_or_set_test() {
        ...
        assert_eq!(
            media_update.add_tags.unwrap().get(0).unwrap().id,
            "test".to_owned()
        );
    }
    #[tokio::test]
    async fn add_or_set_test_with_value() {
        ...
        assert_eq!(
            media_update.add_tags.as_ref().unwrap().get(0).unwrap().id,
            "exist".to_owned()
        );
        assert_eq!(
            media_update.add_tags.as_ref().unwrap().get(1).unwrap().id,
            "test".to_owned()
        );
    }
...
pub fn value_to_hashmap(value: Value) -> RsResult<HashMap<String, String>> {
    if let Value::Object(map) = value {
        ...
        Err(Error::Error(
            "Invalid body for hashmap:Body is not JSON".to_string(),
        ))
    }
}
