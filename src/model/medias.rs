...
impl ModelController {
    ...
    /// Check if a media with the same origin already exists, and send duplicate notification if so
    async fn check_origin_duplicate(
        &self,
    ...
    /// Check if a media with the same hash already exists, and remove the source file if so
    async fn check_hash_duplicate(
        &self,
        library_id: &str,
        store: &crate::model::store::sql::library::SqliteLibraryStore,
        source_path: &str,
    ...
    /// Try to load a thumbnail from a URL request
    async fn load_thumbnail_from_url(
        &self,
        library_id: &str,
        media_id: &str,
        thumb_url: &str,
        first_request: Option<&RsRequest>,
    ) -> bool {
    ...
    /// Handle downloads for virtual libraries - stores URL references instead of actual files
    async fn download_virtual_library(
        &self,
        library_id: &str,
        library: &crate::model::libraries::ServerLibraryForRead,
        files: RsGroupDownload,
        store: &crate::model::store::sql::library::SqliteLibraryStore,
        requesting_user: &ConnectedUser,
    ) -> RsResult<Vec<Media>> {
    ...
    /// Handle grouped downloads - creates a zip file from multiple requests
    async fn download_grouped(
        &self,
        library_id: &str,
        files: RsGroupDownload,
        ...
        let len = files.requests.len() as u64;
    ...
    async fn download_individual(
        ...
        let mut medias: Vec<Media> = vec![];
        let request_count = files.requests.len();
        let mut first_error = None;

        for mut request in files.requests {
            if request.upload_id.is_none() {
                request.upload_id = Some(nanoid!());
            }
                ...
                .await
            {
                ...
                Err(error) => {
                    let is_duplicate = matches!(&error, RsError::Model(Error::Duplicate(_, _)));
                    ...
                    if is_duplicate {
                        log_warn(
                            LogServiceType::Source,
                            format!(
                                "Skipping duplicate individual download {}: {}",
                                request_url, error_message
                            ),
                        );
                    } else {
                        self.send_individual_download_failure(
                            library_id,
                            &upload_id,
                            filename,
                            &request_url,
            ...
            if let Some(error) = first_error {
                if request_count > 1 {
                    log_error(
                        LogServiceType::Source,
                        format!(
                            "All {} individual downloads failed for library {}",
                            request_count, library_id
                        ),
                    );
                }
                return Err(error);
    ...
    async fn download_individual_request(
        &self,
        library_id: &str,
        library: &crate::model::libraries::ServerLibraryForRead,
        ...
        let write_result: RsResult<()> = if let Some(ref key) = encryption_key {
            match CtrEncryptWriter::new(file, key) {
                Ok(mut enc_file) => {
        ...
        if let Err(error) = write_result {
            if let Err(cleanup_error) = m.remove(&source).await {
                log_error(
                    LogServiceType::Source,
                    format!(
                        "Unable to clean failed download {} after write error: {:#}",
    ...
    fn request_progress_filename(request: &RsRequest) -> Option<String> {
        request
            .filename
            .clone()
            .or_else(|| request.selected_file.clone())
            .or_else(|| filename_from_path(&request.url))
    ...
    async fn send_individual_download_failure(
        ...
        log_error(
            LogServiceType::Source,
            format!(
                "Unable to download individual media request {}: {}",
                request_url, error_message
            ),
        );
