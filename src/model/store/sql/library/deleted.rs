...
impl SqliteLibraryStore {
    ...
    pub async fn get_deleted(&self, query: DeletedQuery) -> Result<Vec<RsDeleted>> {
        let row = self
            ...
            .call(move |conn| {
                ...
                let backups: Vec<RsDeleted> =
                    rows.collect::<std::result::Result<Vec<RsDeleted>, rusqlite::Error>>()?;
                Ok(backups)
            })
            .await?;
        Ok(row)
    ...
    pub async fn add_deleted(&self, deleted: RsDeleted) -> Result<()> {
        self.connection
            .call(move |conn| {
                conn.execute(
                    "INSERT OR REPLACE INTO deleted (type, id, date)
            VALUES (?, ? ,?)",
                    params![deleted.kind, deleted.id, deleted.date],
                )?;
