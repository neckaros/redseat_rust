...
use crate::{
    domain::people::{FaceBBox, FaceEmbedding, Person, UnassignedFace},
    model::{
        people::{PeopleQuery, PersonForInsert, PersonForUpdate},
        store::{
...
when (alt like '%|{}|%' or  alt like '{}|%'  or  alt like '%|{}'  or alt = '{}' COLLATE NOCASE ) then 10
    ...
    pub async fn get_person(&self, credential_id: &str) -> Result<Option<Person>> {
        ...
        let row = self
            ...
            .call(move |conn| {
                ...
                let row = query
                    .query_row([credential_id], Self::row_to_person)
                    .optional()?;
                Ok(row)
            })
            .await?;
    ...
    pub async fn update_person(&self, person_id: &str, update: PersonForUpdate) -> Result<()> {
        ...
        let existing = self.get_person(&person_id).await?.ok_or_else(|| {
        ...
        self.connection
            .call(move |conn| {
                ...
                let socials = replace_add_remove_from_array(
                    existing.socials,
                    update.socials,
                    update.add_socials,
                    update.remove_socials,
                );
                let socials = optional_serde_to_string(socials)
    ...
    pub async fn update_person_portrait(&self, id: String) -> Result<()> {
        self.connection
            .call(move |conn| {
                conn.execute(
                    "update people set posterv = ifnull(posterv, 0) + 1 WHERE id = ?",
                    params![id],
                )?;
                Ok(())
            })
    ...
    pub async fn remove_person(&self, tag_id: String) -> Result<()> {
        self.connection
            .call(move |conn| {
                conn.execute(
                    "DELETE FROM media_people_mapping WHERE people_ref = ?",
                    &[&tag_id],
                )?;
                conn.execute(
                    "DELETE FROM book_people_mapping WHERE people_ref = ?",
                    &[&tag_id],
                )?;
    ...
    pub async fn get_highest_confidence_face(
        &self,
        person_id: &str,
    ...
    pub async fn get_face_by_id(
        &self,
        face_id: &str,
    ) -> Result<Option<(String, Option<FaceBBox>)>> {
        let fid = face_id.to_string();
        let res = self
            .connection
            .call(move |conn| {
                ...
                let mut stmt =
                    conn.prepare("SELECT media_ref, bbox FROM people_faces WHERE id = ?")?;
                let result: Option<(String, Option<FaceBBox>)> = stmt
                    .query_row(params![fid.clone()], |row| {
                ...
                let mut stmt =
                    conn.prepare("SELECT media_ref, bbox FROM unassigned_faces WHERE id = ?")?;
                let result: Option<(String, Option<FaceBBox>)> = stmt
    ...
    pub async fn assign_cluster_to_faces(
        &self,
        face_ids: Vec<String>,
        cluster_id: String,
    ) -> Result<()> {
            ...
            .call(move |conn| {
                ...
                let tx = conn.transaction()?;
                {
                    let mut stmt = tx.prepare(
                        "UPDATE unassigned_faces SET cluster_id = ?, processed = 1 WHERE id = ?",
                    )?;
                    for fid in face_ids {
    ...
    pub async fn mark_faces_as_processed(&self, face_ids: Vec<String>) -> Result<()> {
        self.connection
            .call(move |conn| {
                ...
                {
                    let mut stmt =
                        tx.prepare("UPDATE unassigned_faces SET processed = 1 WHERE id = ?")?;
                    for fid in face_ids {
    ...
    pub async fn promote_cluster_to_person(
        &self,
        cluster_id: String,
        person_id: String,
    ) -> Result<()> {
    ...
    pub async fn assign_unassigned_faces_to_person_batch(
        &self,
        face_ids: Vec<String>,
        person_id: String,
    ) -> Result<usize> {
    ...
    pub async fn mark_media_face_processed(&self, media_id: String) -> Result<()> {
        self.connection
            .call(move |conn| {
                conn.execute(
                    "UPDATE medias SET face_processed = 1 WHERE id = ?",
                    params![media_id],
                )?;
                Ok(())
            })
    ...
    pub async fn mark_media_face_error(&self, media_id: String, error: String) -> Result<()> {
        self.connection
            .call(move |conn| {
                conn.execute(
                    "UPDATE medias SET face_recognition_error = ? WHERE id = ?",
                    params![error, media_id],
                )?;
                Ok(())
            })
            .await?;
    ...
    pub async fn transfer_faces_between_people(
        &self,
    ...
    pub async fn update_media_people_mapping(
        &self,
        media_id: String,
        person_id: &str,
    ) -> Result<()> {
    ...
    pub async fn count_medias_for_face_processing(&self) -> Result<u64> {
        let res = self
            ...
            .call(move |conn| {
                let mut stmt =
                    conn.prepare("SELECT COUNT(*) FROM medias WHERE face_processed = 0")?;
