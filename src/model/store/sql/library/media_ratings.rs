...
use crate::{
    ...
    model::{
        ...
        store::sql::{OrderBuilder, RsQueryBuilder, SqlWhereType},
    },
};
...
impl SqliteLibraryStore {
        ...
        let row = self
            ...
            .call(move |conn| {
                ...
                let mut query = conn.prepare(&format!(
                    "SELECT type, ref, user_ref, rating, modified FROM ratings {}{}",
                    where_query.format(),
                    where_query.format_order()
                ))?;

                let rows = query.query_map(where_query.values(), Self::row_to_media_rating)?;
                let backups: Vec<RsMediaRating> =
                    rows.collect::<std::result::Result<Vec<RsMediaRating>, rusqlite::Error>>()?;
                Ok(backups)
            })
            .await?;
        Ok(row)
    ...
    pub async fn set_media_rating(
        ...
        self.connection
            .call(move |conn| {
                conn.execute(
                    "INSERT OR REPLACE INTO ratings (type, ref, user_ref, rating)
            VALUES (?, ?, ? ,?)",
                    params![kind, ref_id, user_ref, rating],
                )?;
