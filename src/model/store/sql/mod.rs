...
pub async fn migrate_database(connection: &Connection) -> Result<usize> {
    let version = connection.call( |conn| {
        let version = conn.query_row(
            ...
            if version < 8 {
                let update = String::from_utf8_lossy(include_bytes!("008 - LIBRARY PASSWORD.sql"));
                conn.execute_batch(&update)?;
            ...
            if version < 10 {
                let update = String::from_utf8_lossy(include_bytes!("010 - UPLOAD KEY PEOPLE.sql"));
                conn.execute_batch(&update)?;
...
pub enum SqlWhereType {
    ...
    GreaterOrEqual(String, Box<dyn ToSql>),
    SmallerOrEqual(String, Box<dyn ToSql>),
