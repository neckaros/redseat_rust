...
impl ModelController {
    pub async fn get_face_recognition_service(&self) -> RsResult<Arc<FaceRecognitionService>> {
        {
    ...
    pub async fn add_pesron(
        ...
        let new_person = self
            .get_person(library_id, backup.id.clone(), requesting_user)
            .await?
            .ok_or(SourcesError::UnableToFindPerson(
                library_id.to_string(),
                backup.id,
                "add_pesron".to_string(),
            ))?;
        self.send_people(PeopleMessage {
            library: library_id.to_string(),
            people: vec![PersonWithAction {
    ...
    pub async fn remove_person(
        ...
        requesting_user.check_library_role(library_id, LibraryRole::Admin)?;
        let store = self.store.get_library_store_optional(library_id).ok_or(
            Error::LibraryStoreNotFoundFor(library_id.to_string(), "remove_person".to_string()),
        )?;
        let existing = store
            .get_person(tag_id)
            .await?
            .ok_or(SourcesError::UnableToFindPerson(
                library_id.to_string(),
                tag_id.to_string(),
                "remove_person".to_string(),
            ))?;

        store.remove_person(tag_id.to_string()).await?;
        self.add_deleted(
            library_id,
            RsDeleted::person(tag_id.to_owned()),
            requesting_user,
        )
        .await?;
        self.send_people(PeopleMessage {
                ...
                match refresh_result {
                    ...
                    Err(_) => {
                        ...
                        if let Ok(Some(face)) = store.get_highest_confidence_face(person_id).await {
                            ...
                            if let (Some(media_ref), Some(_bbox)) =
                                (face.media_ref.as_ref(), face.bbox.as_ref())
                            {
                                if !media_ref.is_empty() {
                                    ...
                                    log_info(
                                        crate::tools::log::LogServiceType::Source,
                                        format!(
                                            "Updating person ({}) image from face: {}",
                                            person_id, face.id
                                        ),
                                    );
                                    match self
                                        ...
                                        .await
                                    {
                                        ...
                                        Err(e) => {
                                            log_info(
                                                crate::tools::log::LogServiceType::Source,
                                                format!("Failed to get face image: {}", e),
                                            );
                                        }
                                    }
                                }
    ...
    pub async fn update_person_image<T: AsyncRead>(
        &self,
        library_id: &str,
        person_id: &str,
        kind: &Option<ImageType>,
        ...
        let person = self
            .get_person(library_id, person_id.to_owned(), requesting_user)
            .await?
            .ok_or(Error::PersonNotFound(person_id.to_owned()))?;
        self.send_people(PeopleMessage {
            library: library_id.to_string(),
            people: vec![PersonWithAction {
                person: person.clone(),
    ...
    pub async fn refresh_person_image(
        &self,
        library_id: &str,
        person_id: &str,
        kind: &Option<ImageType>,
    ...
    pub async fn process_media_faces(
        ...
        let service = if let Some(s) = service {
            ...
            self.get_face_recognition_service().await?
        };
        ...
        let result: RsResult<Vec<DetectedFaceResult>> = async {
            ...
            if media.kind == FileType::Video {
                ...
                let faces = service
                    .detect_and_extract_faces_async(image_result.image)
                    .await?;
                for face in faces {
                    let bbox = FaceBBox {
                        x1: face.bbox.x1,
            ...
            for collected_face in deduplicated_faces {
                ...
                if let Some((person_id, sim)) = self
                    .match_face_to_person(library_id, &collected_face.face.embedding, threshold)
                    .await?
                {
                    ...
                    store
                        .add_face_embedding(
                            face_id.clone(),
                            &person_id,
                            collected_face.face.embedding.clone(),
                            Some(media_id.to_string()),
                            Some(collected_face.bbox.clone()),
                            collected_face.face.confidence,
                            Some(collected_face.face.pose),
                            Some(sim),
                        )
                        .await?;
        ...
        match result {
            Ok(res) => {
                store
                    .mark_media_face_processed(media_id.to_string())
                    .await?;
                Ok(res)
            }
            Err(e) => {
                ...
                crate::tools::log::log_error(
                    ...
                    format!("Error processing faces for media {}: {}", media_id, err_msg),
                );
                store
                    .mark_media_face_error(media_id.to_string(), err_msg)
                    .await?;
                Err(e)
            }
        }
    ...
    pub async fn cluster_unassigned_faces(
        ...
        .await
        .map_err(|e| RsError::Error(format!("Clustering task failed: {}", e)))?;
        ...
        for (cluster_id, face_ids) in clusters {
            ...
            if unique_media_count >= 10 {
                ...
                store
                    .assign_cluster_to_faces(face_ids.clone(), cluster_id.clone())
                    .await?;
                ...
                let person = self
                    .add_pesron(library_id, new_person, &ConnectedUser::ServerAdmin)
                    .await?;
                store
                    .promote_cluster_to_person(cluster_id, person.id)
                    .await?;
                ...
                let cluster_media_ids: Vec<String> = face_ids
                    ...
                    .into_iter()
                    .collect();
                self.send_media_update_events(library_id, &cluster_media_ids, requesting_user)
                    .await;
        ...
        let singleton_face_ids: Vec<String> = all_face_ids
            .iter()
        ...
        if !singleton_face_ids.is_empty() {
            crate::tools::log::log_info(
                crate::tools::log::LogServiceType::Other,
                format!(
                    "Marking {} singleton faces as processed",
                    singleton_face_ids.len()
                ),
            );
    ...
    pub async fn get_unassigned_faces(
        &self,
        library_id: &str,
        requesting_user: &ConnectedUser,
    ) -> RsResult<Vec<UnassignedFace>> {
        requesting_user.check_library_role(library_id, LibraryRole::Read)?;
        let store = self.store.get_library_store(library_id)?;
    ...
    pub async fn get_all_unassigned_faces(
        &self,
        library_id: &str,
        limit: Option<usize>,
        ...
        let store = self.store.get_library_store(library_id)?;
        let faces = store
            .get_all_unassigned_faces(limit, created_before)
            .await?;
        Ok(faces)
    }
    ...
    pub async fn delete_face(
        ...
        if deleted {
            if let Err(e) = self
                .remove_library_image(library_id, ".faces", face_id, &None, &None, requesting_user)
                .await
            {
                log_info(
                    crate::tools::log::LogServiceType::Other,
    ...
    pub async fn assign_unassigned_face_to_person(
        &self,
        library_id: &str,
        face_id: &str,
        person_id: &str,
        ...
        let media_ids = self
            .get_media_ids_from_face_ids(library_id, &[face_id.to_string()], true)
            .await?;

        let store = self.store.get_library_store(library_id)?;
        store
            .assign_unassigned_face_to_person(face_id.to_string(), person_id.to_string())
            .await?;
    ...
    pub async fn assign_unassigned_faces_to_person(
        &self,
        library_id: &str,
        face_ids: &[String],
        person_id: &str,
        ...
        // Get media IDs before assignment
        let media_ids = self
            .get_media_ids_from_face_ids(library_id, face_ids, true)
            .await?;

        let store = self.store.get_library_store(library_id)?;
        let count = store
            .assign_unassigned_faces_to_person_batch(face_ids.to_vec(), person_id.to_string())
            .await?;
    ...
    pub async fn unassign_faces_from_person(
        &self,
        library_id: &str,
        face_ids: &[String],
        requesting_user: &ConnectedUser,
        ...
        let media_ids = self
            .get_media_ids_from_face_ids(library_id, face_ids, false)
            .await?;

        let store = self.store.get_library_store(library_id)?;
        let count = store
            .unassign_faces_from_person_batch(face_ids.to_vec())
            .await?;
    ...
    pub async fn match_unassigned_faces_to_people(
        &self,
        library_id: &str,
        requesting_user: &ConnectedUser,
    ) -> RsResult<usize> {
        ...
        for face in unassigned_faces {
            ...
            match self
                .match_face_to_person(library_id, &face.embedding, threshold)
                .await
            {
                Ok(Some((person_id, _sim))) => {
                    // Match found - assign the face to this person
                    match self
                        .assign_unassigned_face_to_person(
                            library_id,
                            &face.id,
                            &person_id,
                            requesting_user,
                        ...
                        .await
                    {
                        ...
                        Err(e) => {
                            crate::tools::log::log_error(
                                ...
                                format!(
                                    "Error assigning face {} to person {}: {:#}",
                                    face.id, person_id, e
                                ),
                            );
                        }
    ...
    pub async fn get_face_image(
        ...
        if self
            .has_library_image(library_id, ".faces", face_id, None, requesting_user)
            .await?
        {
            // Load cached image
            let mut cached_image = self
                .library_image(library_id, ".faces", face_id, None, None, requesting_user)
                .await?;
        ...
        let (media_ref, bbox) = store
            .get_face_by_id(face_id)
            .await?
        ...
        let (cropped, _offset_x, _offset_y) =
            FaceRecognitionService::crop_face_with_padding(&image, &bbox_for_crop, 0.3)?;

        // Resize to match person image sizing and convert to AVIF format
        use crate::tools::image_tools::{save_image_native, ImageAndProfile};
            ...
            .await
        {
            ...
            log_info(
                crate::tools::log::LogServiceType::Other,
                format!("Failed to cache face image {}: {}", face_id, e),
            );
        }
    ...
    async fn get_media_ids_from_face_ids(
        &self,
        library_id: &str,
        face_ids: &[String],
    ...
    async fn send_media_update_events(
        &self,
        library_id: &str,
        media_ids: &[String],
        requesting_user: &ConnectedUser,
    ...
    pub async fn merge_people(
        ...
        let source_person = self
            .get_person(library_id, source_person_id.to_string(), requesting_user)
            .await?
            .ok_or_else(|| RsError::NotFoundPerson(source_person_id.to_string()))?;
        let target_person = self
        ...
        // Transfer faces
        let store = self.store.get_library_store(library_id)?;
        let faces_transferred = store
            .transfer_faces_between_people(source_person_id, target_person_id)
            .await?;

        // Delete source person (this also checks Admin permissions internally)
        self.remove_person(library_id, source_person_id, requesting_user)
            .await?;

        Ok(faces_transferred)
    }

    pub async fn get_medias_for_face_processing(
        &self,
        library_id: &str,
        limit: usize,
    ) -> RsResult<Vec<String>> {
    ...
    pub async fn count_medias_for_face_processing(
        &self,
        library_id: &str,
        requesting_user: &ConnectedUser,
    ) -> RsResult<u64> {
    ...
    async fn match_face_to_person(
        &self,
        library_id: &str,
        embedding: &[f32],
        threshold: f32,
    ) -> RsResult<Option<(String, f32)>> {
        let store = self.store.get_library_store(library_id)?;
    ...
    for i in 0..n {
        for j in (i + 1)..n {
            let sim = cosine_similarity(
                &collected_faces[i].face.embedding,
                &collected_faces[j].face.embedding,
            );
            if sim >= threshold {
                adj[i].push(j);
                adj[j].push(i);
...
pub fn chinese_whispers_clustering(
    ...
    crate::tools::log::log_info(
        crate::tools::log::LogServiceType::Other,
        format!(
            "Clustering {} unassigned faces with threshold {}",
            n, threshold
        ),
    );
    ...
    crate::tools::log::log_info(
        crate::tools::log::LogServiceType::Other,
        format!(
            "Formed {} edges (similarity >= {})",
            edges_formed, threshold
        ),
    );
    ...
    if !similarities.is_empty() {
        ...
        crate::tools::log::log_info(
            crate::tools::log::LogServiceType::Other,
            format!(
                "Similarity stats: min={:.3}, max={:.3}, avg={:.3}",
    ...
    for iter in 0..iterations {
        ...
        if changes == 0 {
            crate::tools::log::log_info(
                crate::tools::log::LogServiceType::Other,
                format!("Clustering converged after {} iterations", iter + 1),
            );
            break;
        }
    }
    ...
    crate::tools::log::log_info(
        crate::tools::log::LogServiceType::Other,
        format!(
            "After filtering singletons: {} clusters remaining",
            clusters.len()
        ),
    );
