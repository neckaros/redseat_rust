...
use crate::{
    domain::{library::LibraryRole, media::Media, MediaElement},
    error::{ClientError, DuplicateClientError},
    plugins::sources::error::SourcesError,
    tools::image_tools::ImageError,
};

use super::{
    libraries::ServerLibraryForUpdate,
    users::{ConnectedUser, ServerUser, ServerUserForUpdate, UserRole},
};
...
pub enum Error {
    ...
    UserUpdateNotAuthorized {
        user: ServerUser,
        update_user: ServerUserForUpdate,
    },
    UserRoleUpdateNotAuthOnlyAdmin,
    LibraryUpdateNotAuthorized {
        user: ServerUser,
        update_library: ServerLibraryForUpdate,
    },
    ...
    pub fn client_status_and_error(&self) -> (StatusCode, ClientError) {
        ...
        match self {
            ...
            Error::LibraryDeletionInProgress(_) => (
                StatusCode::CONFLICT,
                ClientError::Custom("Library deletion already in progress".to_string()),
            ),

            Self::Duplicate(id, element) => (
                StatusCode::NOT_FOUND,
                ClientError::DUPLICATE(DuplicateClientError {
                    id: id.to_string(),
            ...
            Error::InvalidIdForAction(action, id) => (
                ...
                ClientError::Custom(format!("Invalid id {} for {}", id, action)),
            ),
            ...
            Error::InsufficientUserRole { user: _, role: _ } => {
                (StatusCode::FORBIDDEN, ClientError::FORBIDDEN)
            }
            Error::InsufficientLibraryRole {
                user: _,
                library_id: _,
                role: _,
            } => (StatusCode::FORBIDDEN, ClientError::FORBIDDEN),
            Error::UserGetNotAuth {
                user: _,
                requested_user: _,
            } => (StatusCode::FORBIDDEN, ClientError::FORBIDDEN),
            ...
            Error::UserRoleUpdateNotAuthOnlyAdmin => {
                (StatusCode::FORBIDDEN, ClientError::FORBIDDEN)
            }
            Error::LibraryUpdateNotAuthorized {
                user: _,
                update_library: _,
            } => (StatusCode::FORBIDDEN, ClientError::FORBIDDEN),

            Error::Rusqlite(_) | Error::TokioRusqlite(_) => (
                StatusCode::INTERNAL_SERVER_ERROR,
                ClientError::SERVICE_ERROR,
            ),
            Error::Serde(_) => (
