...
use tokio::{
    fs::{self, File},
    io::{copy, AsyncRead, AsyncReadExt, AsyncWriteExt, BufReader},
    sync::mpsc,
};
...
use crate::{
    domain::{
        backup::{
            self, Backup, BackupError, BackupFile, BackupFileProgress, BackupMessage,
            BackupProcessStatus, BackupStatus, BackupWithStatus,
        },
        library::{LibraryRole, ServerLibrary},
        media::{self, Media, MediaForUpdate, DEFAULT_MIME},
        progress::{RsProgress, RsProgressType},
    },
    error::{RsError, RsResult},
    model::libraries::ServerLibraryForAdd,
    plugins::sources::{
    ...
    tools::{
        clock::now,
        encryption::{
            ceil_to_multiple_of_16, derive_key, estimated_encrypted_size, random_iv,
...
impl ModelController {
    ...
    pub async fn get_backups_with_status(
        &self,
        requesting_user: &ConnectedUser,
    ) -> Result<Vec<BackupWithStatus>> {
        requesting_user.check_role(&UserRole::Admin)?;
    ...
    pub async fn get_backup_with_status(
        &self,
        backup_id: &str,
        requesting_user: &ConnectedUser,
    ) -> Result<Option<BackupWithStatus>> {
        requesting_user.check_role(&UserRole::Admin)?;
    ...
    pub async fn get_backup_files_infos(
        &self,
        backup_id: &str,
        requesting_user: &ConnectedUser,
    ) -> Result<BackupInfos> {
    ...
    pub async fn get_backup_media_backup_files(
        ...
        let backup_files = self
            .store
            .get_backup_media_backup_files(&backup_id, &media_id)
            .await?;
        Ok(backup_files)
    }

    /// For a specific media id get all the files for a specific library whatever the backup
    pub async fn get_library_media_backup_files(
        &self,
        library_id: &str,
        media_id: &str,
        requesting_user: &ConnectedUser,
        ...
        let backup_files = self
            .store
            .get_library_media_backup_files(&library_id, &media_id)
            .await?;
        Ok(backup_files)
    }
    ...
    ///
    /// UNPROTECTED INTERNAL USE ONLY
    pub async fn check_backup_existing_media(
        &self,
        backup_id: &str,
        media_id: &str,
        source_hash: &str,
    ...
    pub async fn get_backup_file(&self, backup_file_id: &str) -> RsResult<BackupFile> {
        let backup = self
            .store
            .get_backup_file(backup_file_id)
            .await?
            .ok_or(RsError::BackupFileNotFound(backup_file_id.to_owned()))?;
    ...
    pub async fn get_backup_media(
        &self,
        library_id: &str,
        media_id: &str,
        backup_file_id: Option<&str>,
        requesting_user: &ConnectedUser,
    ) -> RsResult<SourceRead> {
        requesting_user.check_library_role(&library_id, LibraryRole::Admin)?;
        ...
        let backup_file = if let Some(backup_file_id) = backup_file_id {
            ...
            let backup_files = self
                .get_library_media_backup_files(library_id, media_id, requesting_user)
                .await?;
            backup_files
                .first()
                .ok_or(RsError::BackupNotFound(
        ...
        let backup_info = self
            .get_backup(&backup_file.backup, requesting_user)
            .await?
            .ok_or(RsError::BackupProcessNotFound(
                backup_file.backup.to_string(),
            ))?;

        let source = self
            .plugin_manager
            .provider_for_backup(backup_info.clone(), self.clone())
            .await?;
    ...
    pub async fn check_role_for_backup_id(
        &self,
        backup_file_id: &str,
        requesting_user: &ConnectedUser,
    ) -> RsResult<()> {
    ...
    ///
    /// If no `backup_file_id` is provided, the last backup will be fetched
    pub async fn get_backup_file_reader(
        &self,
        backup_file_id: &str,
        requesting_user: &ConnectedUser,
    ) -> RsResult<SourceRead> {
        ...
        let source = if let Some(password) = backup_info.password.clone() {
            ...
            let media = if let Some(library_id) = backup_file.library {
                self.get_media(&library_id, backup_file.file.clone(), requesting_user)
                    .await?
            } else {
                None
            };
    ...
    pub async fn upload_backup(
        ...
        let backup_info = self
            .get_backup(&backup_id, &ConnectedUser::ServerAdmin)
            .await?
            .ok_or(RsError::BackupProcessNotFound(backup_id.to_string()))?;

        let media_info_string = if let Some(infos) = infos.as_ref() {
            Some(serde_json::to_string(infos)?)
        } else {
            None
        };
        let media_info_string_len = media_info_string.as_ref().map(|i| i.len() as u64);
        let media_info_string_enc_len =
            media_info_string_len.map(|l| ceil_to_multiple_of_16(l as usize) as u64);

        let initial_message = BackupFileProgress::new(
            file_id.clone(),
            backup_id.clone(),
            library_id.clone(),
            source_read.filename(),
            source_read.size(),
            id.to_string(),
        ...
        let backup = if let Some(password) = backup_info.password.clone() {
            ...
            let iv = random_iv();
            let encrypted_size = if let Some(source_size) = source_size {
                Some(estimated_encrypted_size(
                    source_size,
                    0,
                    media_info_string_len.clone().unwrap_or(0),
                ))
            } else {
                None
            };
            let writer = provider
                .writer(&id, encrypted_size, source_mime.clone())
                .await?;
            let mut crypt_stream = AesTokioEncryptStream::new(
                writer.1,
                &key,
                &iv,
                source_mime.clone(),
            ...
            let backup = BackupFile {
                id,
                backup: backup_info.id,
                library: backup_info.library,
                file: file_id,
                path: backup_file_source,
                hash: infos.md5.clone().unwrap_or_default(),
                sourcehash,
                size: source_size.unwrap_or_default(),
                modified,
                added: now().timestamp_millis(),
                iv: Some(hex::encode(&iv)),
                thumb_size: None,
                info_size: media_info_string_enc_len,
                error: None,
            };
            backup
        } else {
            ...
            let backup = BackupFile {
                id,
                backup: backup_info.id,
                library: backup_info.library.clone(),
                file: file_id,
                path: backup_file_source,
                hash: infos.md5.clone().unwrap_or_default(),
                sourcehash,
    ...
    pub async fn upload_backup_media(
        &self,
        backup_id: &str,
        library_id: &str,
        media_id: &str,
        ...
        let media_info = self
            ...
            .ok_or(SourcesError::UnableToFindMedia(
                library_id.to_string(),
                media_id.to_string(),
                "upload_backup_media".to_string(),
            ))?
            .item;
        let mut source_read = self
            .library_file(
                &library_id,
                media_id,
                None,
    ...
    pub async fn upload_backup_path(
        &self,
        backup_info: Backup,
        file_id: &str,
        path: PathBuf,
        name: String,
        library: Option<ServerLibrary>,
    ) -> RsResult<BackupFile> {
        let sourcehash = {
        ...
        let id = nanoid!();
        let existing_db_bakcups = self
            .get_backup_media_backup_files(&backup_info.id, file_id, &ConnectedUser::ServerAdmin)
            .await?;

        let exist = existing_db_bakcups
            .into_iter()
            .find(|b| b.sourcehash == sourcehash);

        if let Some(exist) = exist {
            log_info(
                crate::tools::log::LogServiceType::Scheduler,
                format!("Backup {} identical already uploaded. Ignoring", file_id),
            );
            Ok(exist)
        } else {
            ...
            let reader = if let Some(library) = library {
                let library_add = ServerLibraryForAdd {
                    name: library.name,
            ...
            let backup_file = self
                .upload_backup(
                    source_read,
                    file_id.to_string(),
                    backup_info.id.clone(),
                    sourcehash,
                    now().timestamp_millis(),
                    backup_info.library.clone(),
                    None,
                    Some(id),
                )
                .await?;
            ...
            let backup_file = self
                .add_backup_file(backup_file, &ConnectedUser::ServerAdmin)
                .await?;
            println!("upload path");
            Ok(backup_file)
        }
    ...
    pub async fn remove_backup_file(
        ...
        self.store
            .remove_backup_file(backup_file.id.to_owned())
            .await?;
        Ok(())
    }
    ...
    pub async fn remove_backup_files_for_media(
        &self,
        backup_id: &str,
        media_id: &str,
        older_than: Option<i64>,
        not_id: Option<String>,
        requesting_user: &ConnectedUser,
    ) -> RsResult<usize> {
        let backup_files = self
            .get_backup_media_backup_files(backup_id, media_id, requesting_user)
            .await?;
        ...
        for backup_file in backup_files {
            ...
            let r = self
                .remove_backup_file(&backup_file.id, requesting_user)
                .await;
            match r {
                ...
                Err(_) => log_error(
                    crate::tools::log::LogServiceType::Scheduler,
                    format!(
                        "Could not delete backup {} of file {}",
                        backup_file.id, backup_file.file
                    ),
                ),
            }
        }
