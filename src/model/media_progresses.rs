...
impl ModelController {
    ...
    pub async fn get_media_progress(&self, library_id: &str, media_ref: String, requesting_user: &ConnectedUser) -> RsResult<RsMediaProgress> {
        let progress = self.get_medias_progresses(library_id, MediaProgressesQuery { media: Some(media_ref), ..Default::default() }, requesting_user).await?;
        let p = progress.into_iter().next().ok_or(RsError::NotFound)?;
		Ok(p)
	}
