...
use crate::{
    domain::{
        deleted::RsDeleted, episode::Episode, library::LibraryRole,
        media_progress::RsMediaProgress, media_rating::RsMediaRating, progress,
    },
...
impl ModelController {
    pub async fn send_media_progress(&self, message: MediasProgressMessage) {
        let mapping: Vec<crate::domain::library::UserMapping> = self
            .get_library_mapped_users(&message.library)
            .await
            .ok()
            .unwrap_or_default();
    ...
    ) -> RsResult<Vec<RsMediaProgress>> {
        let original_user_id = requesting_user.user_id()?;
        let mut user_id = self
            .get_library_mapped_user(library_id, original_user_id.clone())
            .await?;
    ...
    pub async fn get_media_progress(
        ...
        media_ref: String,
        requesting_user: &ConnectedUser,
    ) -> RsResult<RsMediaProgress> {
        let progress = self
            .get_medias_progresses(
                library_id,
                MediaProgressesQuery {
                    media: Some(media_ref.clone()),
                    ..Default::default()
                },
                requesting_user,
            )
        ...
        let p = progress
            .into_iter()
            .next()
            .ok_or(RsError::NotFound(format!(
                "Media progress not found: {} for user {:?}",
                media_ref, requesting_user
            )))?;
        Ok(p)
    }
    ...
    pub async fn set_media_progress(
        ...
        let store = self.store.get_library_store(library_id)?;
        store
            .set_media_progress(media_ref.clone(), user_id, progress)
            .await?;
        let progress = self
            .get_media_progress(library_id, media_ref, requesting_user)
            .await?;
