...
use rs_plugin_common_interfaces::{
    video::{
        RsVideoCapabilities, RsVideoTranscodeCancelResponse, RsVideoTranscodeJob,
        RsVideoTranscodeJobPluginAction, RsVideoTranscodeJobPluginRequest,
        RsVideoTranscodeJobStatus, VideoConvertRequest,
    },
    CustomParamTypes, PluginCredential, PluginType, RsRequest,
};
use serde_json::{Map, Value};

use crate::{
    domain::{
        plugin::{self, PluginWithCredential},
        progress::RsProgress,
    },
    error::RsResult,
    model::{
...
fn plugin_param_value(param: &CustomParamTypes) -> Option<Value> {
    let value = serde_json::to_value(param).ok()?;
    match value {
        Value::Object(object) if object.len() == 1 => {
            object.into_iter().next().map(|(_, value)| value)
        }
        value => Some(value),
    }
}

fn credential_with_plugin_settings(plugin_with_cred: &PluginWithCredential) -> PluginCredential {
    let mut credential = plugin_with_cred
        .credential
        .clone()
        .map(PluginCredential::from)
        .unwrap_or_default();

    let mut settings = match credential.settings {
        Value::Object(settings) => settings,
        _ => Map::new(),
    };

    for param in &plugin_with_cred.plugin.params {
        if let Some(value) = plugin_param_value(&param.param) {
            if !value.is_null() {
                settings.insert(param.name.clone(), value);
            }
        }
    }

    credential.settings = Value::Object(settings);
    credential
}

/// Capabilities for video conversion plugins
impl ModelController {
    pub async fn get_all_convert_capabilities(
        &self,
        ...
        let plugins = self
            ...
            .await?;
        self.plugin_manager
            .get_all_convert_capabilities(plugins)
            .await
    }
...
impl PluginManager {
                    ...
                    if let Ok(Json(res)) = res {
                        ...
                        if code != 404 {
                            log_error(
                                ...
                                format!(
                                    "Error request get_convert_capabilities {} {:?}",
                                    code, error
                                ),
                            )
                        }
    ...
    pub async fn get_convert_capabilities(
        &self,
        ...
        if let Some(plugin) = self.plugin_by_filename(&plugin_with_cred.plugin.path).await {
                ...
                .contains(&PluginType::VideoConvert)
            {
                ...
                let res = plugin
                    ...
                    .await
                    .map_err(|error| {
                        plugin_call_error(&plugin.infos.name, "get_convert_capabilities", error)
                    })?;
                return Ok(res.into_inner());
            } else {
                return Err(crate::error::RsError::PluginUnsupportedCall(
                    plugin_with_cred.plugin.id,
                    "get_convert_capabilities".to_string(),
                ));
            }
        } else {
            return Err(crate::error::RsError::PluginNotFound(
                plugin_with_cred.plugin.id,
            ));
        }
    }
...
impl ModelController {
    ...
    pub async fn convert_submit_media(
        &self,
        library_id: &str,
        media_id: &str,
        ...
        let temp_url =
            ModelController::get_temporary_read_url(library_id, media_id, Some(21600)).await?;
        let media = self
            .store
        ...
        let jobrequest = RsVideoTranscodeJobPluginRequest {
            job: RsVideoTranscodeJob { source, request },
            credentials: plugin_with_credentials
                .credential
                .clone()
                .map(|r| r.into())
                .unwrap_or_default(),
        ...
        self.plugin_manager
            .convert_submit(plugin_with_credentials, jobrequest)
            .await
    }
}
impl PluginManager {
        ...
        if let Some(plugin) = self.plugin_by_filename(&plugin_with_cred.plugin.path).await {
                ...
                .contains(&PluginType::VideoConvert)
            {
                ...
                let res = plugin
                    ...
                    .await
                    .map_err(|error| {
                        plugin_call_error(&plugin.infos.name, "convert_link", error)
                    })?;
                return Ok(res.into_inner());
            } else {
                return Err(crate::error::RsError::PluginUnsupportedCall(
                    plugin_with_cred.plugin.id,
                    "convert_link".to_string(),
                ));
            }
        } else {
            return Err(crate::error::RsError::PluginNotFound(
